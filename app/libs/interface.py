...
from app.settings.configs import PLAYER_IN_ROOM
...
class Front_screen(tk.Tk):
    def __init__(self):
        ...
        self.player_in_room = []
        self.policy = False
        self.path_images = os.path.join(".", "app", "images", "uses")
        self.images = {
            "trophy":ImageTk.PhotoImage(Image.open(os.path.abspath(f"{self.path_images}/trophy.png"))),
            "tick":ImageTk.PhotoImage(Image.open(os.path.abspath(f"{self.path_images}/tick.png"))),
        }
    ...
    def add_score(self, period, *, skip=0, **val):
        [txt.config(fg="gray") for txt in self.lastadd] if self.lastadd else None
        self.lastadd = []
        for title in self.board:
            row = len(self.widgets["Scoreboard"]["log"][title])
            print(title)
            print(val[title])
            add = [
                tk.Label(
					...
					text = f"{score}",
					width = 6,
					fg = self.column_color(self.sideColumns[i]),
				) for i, score in enumerate(val[title])
            ] if title != "Main" else []
            self.widgets["Scoreboard"]["log"][title][period] = add
            start = {
                    "A":0,
                    "Main":self.headsize[0]+1,
                    "B":sum(self.headsize)+2
                }
            [
                (
                    self.font_config(score[1], size=10),
                    score[1].grid(  row = row,
                                    column = score[0]+start[title],
                ),
                ) for score in enumerate(add)
            ] if add else None
            self.lastadd = self.lastadd + (add if add else [])
        [
			score.config(text=f"{[*val['A'], *val['B']][i]}")
			for i, score in enumerate(self.widgets["Scoreboard"]["Output"])
		]
        self.widgets["Title"]["day"]["text"] = f"DAY: {val['Main'][0]}"
        self.widgets["Title"]["demand"]["text"] = f"DEMAND: {val['Main'][1]}"
        self.scrollto(1)
    ...
    def server_check_player_join(self, player, *_, check=0):
        logger.debug(f"server_check_player_join 1 | player: {player} | check: {check} | self.team: {self.team}")
        if check < len(self.team):
            if not self.player[self.team[check]]:
                self.player[self.team[check]] = player
                self.player_in_room = self.player_in_room + [player]
                logger.debug(f"team {self.team[check]} is joined")
            else:
                logger.debug(f"team {self.team[check]} is already joined")
                self.server_check_player_join(player, *_, check=check+1)
            
        self.update()
        logger.debug(f"server_check_player_join 2 | player: {player} | check: {check} | self.team: {self.team}")
    
    def starting(self):
        logger.debug(f"starting | starting game | self.player_in_room: {self.player_in_room} | self.team: {self.team}")
        if len(self.player_in_room) == 2:
            self.team_name(**self.player)
            massege = "game starting..."
...
class User_GUI(tk.Tk):
    def __init__(self):
        ...
        self.policy = False
        self.initial = True
        self.path_sounds = os.path.join(".", "app", "sounds")
        self.widgets = {
            "Title":{}
        }
        self.output = {
        ...
        self.widget_position = {
            ...
            "passBox":{"relx":.65, "rely":.6, "anchor":tk.CENTER},
            ...
            "passCode":{"relx":.4, "rely":.6, "anchor":tk.E},
            ...
            "loginButton":{"relx":.875, "rely":.6, "anchor":tk.CENTER},
        ...
        self.path_images = os.path.join(".", "app", "images", "uses")
        real_path_images = os.listdir(os.path.abspath(self.path_images))
        self.images = {
            "box":[
                ImageTk.PhotoImage(Image.open(os.path.abspath(f"{self.path_images}/"+img)))
                for img in real_path_images
            ...
            "cross":ImageTk.PhotoImage(Image.open(os.path.abspath(f"{self.path_images}/cross.png"))),
            "truck":ImageTk.PhotoImage(Image.open(os.path.abspath(f"{self.path_images}/truck.png"))),
            "warehouse":ImageTk.PhotoImage(Image.open(os.path.abspath(f"{self.path_images}/warehouse.png"))),
        }
    ...
    Entry
        ...
        self.widgets["passBox"] = tk.Entry(self)
        ...
        self.widgets["passBox"].config(
            validate="key",
            validatecommand=(
                self.register(lambda val: (len(val)<=6)*all([
                    str in "0123456789"
                    for str in val
                ]) if len(val) else True),
                "%P",
        ...
        self.font_config(self.widgets["passBox"], size=30)
    ...
    def waitingscreen(self):
        ...
        self.passcode = self.widgets["passBox"].get()
        [
            widget.place_forget()
            for widget in self.winfo_children()
        ]
    ...
    def loginscreen(self):
        ...
        self.widgets["passCode"].place(**self.widget_position["passCode"])
        self.widgets["passBox"].place(**self.widget_position["passBox"])
    ...
    def updatestock(self, skip=0):
        ...
        if not skip:
            self.updateimage()
            try:
                playsound(os.path.abspath(f"{self.path_sound}/zapsplat_foley_cardboard_box_small_empty_set_down_or_drop_on_hard_ground_006_85860.mp3"), block=False) if self.sequence["stock"]["box"] else None
                playsound(os.path.abspath(f"{self.path_sound}/zapsplat_household_wicker_basket_hamper_set_down_004_85827.mp3"), block=False) if self.sequence["deli"]*(not last_deli) else None
            except:
        ...
        if not skip:
            self.updateimage()
            try:
                playsound(os.path.abspath(f"{self.path_sound}/zapsplat_foley_notepad_page_thin_paper_a4_ring_binded_rip_tear_from_pad_fast_001_33418.mp3"), block=False) if self.output["receive"] else None
            except:
                print("sound does not play error raised")
            time.sleep(1)
        ...
        try:
            playsound(os.path.abspath(f"{self.path_sound}/Blastwave_FX_CashRegister_S08IN.92.mp3"), block=False) if self.sequence["stock"]["cash"] else None
        except:
            print("sound does not play error raised")
        ...
        self.updateimage()
        try:
            playsound(os.path.abspath(f"{self.path_sound}/zapsplat_multimedia_alert_error_003_26394.mp3"), block=False) if self.sequence["stock"]["lost"] else None
        except:
            print("sound does not play error raised")
    ...
    def font_config(self, widget, **config):
        font = tk_font.Font(font=widget).actual()
        font = {key:config[key] if key in config else font[key] for key in font}
        widget.config(font = tuple(font[key] for key in ["family", "size"]))
